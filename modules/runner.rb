...
while true do
  ...
  # Update the jobs state and close the tap if necessary
   ...
   # Fill job cache if cluster supports it (optimization that limits the number of queries to the cluster's api)
	...
	export2file("Stress",cluster.get_global_stress_factor(),ARGV[0],strlogfile,loop_time)
	...
	nb_allow_jobs = ref - cluster_jobs.select{ |j| j["state"]=="Running" }.length - cluster_jobs.select{ |j| j["state"]=="Waiting" }.length
